...
bool CAcceptIO::OnRecv()
{
	while(true)
	{
		int fd = Accept();
   		...
   		CNetClient* pNetClient = m_pNetService->OnCreateNetClient();
		if(nullptr == pNetClient)
		{
			continue;
		}

		if(!pNetClient->Init(fd, m_pNetService))
		{
			LogError("Socket Init fail.");
		}
		m_pNetService->OnNetConnect(pNetClient);
	}
	return true;
}
