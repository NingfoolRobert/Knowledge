		...
		if(FD_ISSET(m_fd, &fdReadSet))
		{
			...
			while(true)
			{
				...
				else 
				{
					...
					{
						LogError("Recv Data errno:%d.", errno);
						return nullptr;
					}
				}
			}
...
bool CConnection::SendMsg(PHEADER pHeader, bool bAck, int nOrgine, unsigned int  dwAckType, CBuffer* pBuffer)
{
	...
	if(nLen <= 0)
	{
		LogError("Connection fail. IP:Port=%s:%d", m_pAddrInfo->szIP, m_pAddrInfo->nPort);
		return false;
	}
		...
		if(FD_ISSET(m_fd, &ReadSet))
		{
			while(true)
			{
				...
				else if(nLen < 0)
				{
					...
					{
						LogError("Select: Recv Message error.errno= %d", errno);
						CSocket::SetBlockMode(~O_NONBLOCK);
						return false;
					}
				}
				...
				{
					LogError("Connnection closed. IP:Port=%s:%d", m_pAddrInfo->szIP, m_pAddrInfo->nPort);
					CSocket::SetBlockMode(~O_NONBLOCK);
					return false;
				}
			}
