...
#  add_cppcheck(<target-name> [OUTPUT file] [UNUSED_FUNCTIONS] [STYLE] [POSSIBLE_ERROR] [FORCE] [FAIL_ON_WARNINGS]) -
#    Create a target to check a target's sources with cppcheck and the indicated options
#  add_cppcheck_sources(<target-name> [OUTPUT file] [UNUSED_FUNCTIONS] [STYLE] [POSSIBLE_ERROR] [FORCE] [FAIL_ON_WARNINGS]) -
...
function(add_cppcheck_sources _targetname)
	if(CPPCHECK_FOUND)
		...
		list(FIND _input OUTPUT _output)
		if("${_output}" GREATER "-1")
			message("${_output}")
			list(REMOVE_AT _input ${_output})
			list(GET _input "${_output}" _output_file)
			list(REMOVE_AT _input ${_output})
			list(APPEND _cppcheck_args 2> ${CPPCHECK_OUTPUT_DIRECTORY}${_output_file})
		endif()
...
function(add_cppcheck _name)
	...
	if(CPPCHECK_FOUND)
		...
		list(FIND ARGN OUTPUT _output)
		if("${_output}" GREATER "-1")
			list(REMOVE_AT ARGN ${_output})
			list(GET ARGN "${_output}" _output_file)
			list(REMOVE_AT ARGN ${_output})
			list(APPEND _cppcheck_args 2> ${CPPCHECK_OUTPUT_DIRECTORY}${_output_file})
		endif()
