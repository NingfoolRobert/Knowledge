...
int MasterHardDiskSerial::GetSerialNo(std::string& SerialNumber)
{
	getHardDriveComputerID();
	size_t numberLength = strlen(m_cszHardDriveSerialNumber);
	if (numberLength == 0)
	...
	//std::cout << "size " << serialNumber.size() << std::endl;
	//memcpy(&serialNumber.front(), m_cszHardDriveSerialNumber, serialNumber.size()); //m_cszHardDriveSerialNumber;
	SerialNumber = m_cszHardDriveSerialNumber;
	return 0;
}
...
std::string getHDSerialnum(std::string& shdserialnum)
{
	MasterHardDiskSerial hd;
	char szHdSerialnum[1024];
	hd.GetSerialNo(shdserialnum);
	return shdserialnum;
}