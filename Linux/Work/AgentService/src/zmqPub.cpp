...
template<class T>
CZMQPub::CZMQPub():m_bStop(false), m_ulSerialNum(0)
{
	m_zsock = nullptr;
}
...
bool CZMQPub::Init(std::string strURI)
{
	...
	zsock_bind(m_zsock, strURI.c_str());
...
bool CZMQPub::PostMsg(zmsg_t* msg)
{
	...
	//
	std::std::unique_lock<std::mutext> locker(m_clsLock);
...
void CZMQPub::OnMsg()
{
	while(!m_bStop)
	{
		...
		listMsg.resize(m_listMsg.size_approx());
		int nCount = m_listMsgPub.wait_dequeue_bluk(&listMsg[0], nSize);
		locker.unlock();
		//
		for(int i = 0; i < nCount; ++i)
		{		
			auto pmsg =	m_listMsg.front;
			if(pmsg == nullptr)
				continue;
			zsock_send(m_zsock, pmsg);
			zmsg_destroy(&pmsg);
			pmsg = nullptr;
		}
	}
...
void CZMQPub::Terminate()
{
	...
	int ncount = m_listMsgPub.wait_dequeue_bluk(&listMsg[0], m_listMsgPub.size_approx());
	for(int i = 0; i < ncount; ++i)
	{
		auto pMsg = listMsg[i];
		zmsg_destroy(&pMsg);
...
void CZMQPub::GetPubInfo(CBuffer* pBufInfo)
{
	if(pBufInfo == nullptr)
		return ;
	char szTmp[256] = { 0 };
	sprintf(szTmp, "SerialNUM: %ld, MsgCount: %d", m_ulSerialNum, m_listMsgPub.size_approx());
...
{
	std::std::unique_lock(m_clsSerialNum);
